...
#TUN device wrapper to abstract platform differences
...
def run_client(cfg: dict) -> None:
    ...
    if route_all and sys.platform == "linux":
        click.echo(f"[0xVPN] adding Linux default route via {gateway}...")
        subprocess.run(
            ["ip", "route", "replace", "default", "via", gateway, "dev", tun.name],
            check=True,
        )
        click.echo(f"[0xVPN] Linux route added")

    original_gw = None
    if route_all and sys.platform == "darwin":
        click.echo(f"[0xVPN] detecting current default gateway...")
        ...
        for line in gw_result.stdout.splitlines():
            if "gateway:" in line:
                original_gw = line.split("gateway:")[1].strip()
                break
        click.echo(f"[0xVPN] original gateway: {original_gw}")

        click.echo(f"[0xVPN] adding routes 0.0.0.0/1 and 128.0.0.0/1 via {gateway}...")
        r1 = subprocess.run(["route", "-n", "add", "-net", "0.0.0.0/1",   gateway], capture_output=True, text=True)
        r2 = subprocess.run(["route", "-n", "add", "-net", "128.0.0.0/1", gateway], capture_output=True, text=True)
        click.echo(f"[0xVPN] route 0/1: {r1.stdout.strip() or r1.stderr.strip()}")
        click.echo(f"[0xVPN] route 128/1: {r2.stdout.strip() or r2.stderr.strip()}")

        if original_gw:
            click.echo(f"[0xVPN] adding direct route to VPS {host} via {original_gw}...")
            r3 = subprocess.run(["route", "-n", "add", "-host", host, original_gw], capture_output=True, text=True)
            click.echo(f"[0xVPN] VPS route: {r3.stdout.strip() or r3.stderr.strip()}")

        # Save current DNS and set to 1.1.1.1
        click.echo(f"[0xVPN] configuring DNS...")
        dns_result = subprocess.run(
            ["networksetup", "-getdnsservers", "Wi-Fi"],
            capture_output=True, text=True,
        )
        dns_output = dns_result.stdout.strip()
        if "There aren't any DNS Servers" in dns_output or dns_output == "":
            original_dns = "empty"
        else:
            original_dns = dns_output.splitlines()
        subprocess.run(["networksetup", "-setdnsservers", "Wi-Fi", "1.1.1.1", "8.8.8.8"], check=False)
        click.echo(f"[0xVPN] DNS set to 1.1.1.1 8.8.8.8")
    else:
        original_dns = None

    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.bind(("0.0.0.0", 0))
    click.echo(f"[0xVPN] UDP socket bound")

    click.echo(f"[0xVPN] sending test packet to {remote}...")
    try:
        test = encrypt(key, b"\x45\x00" + b"\x00" * 18)
        sock.sendto(test, remote)
        click.echo(f"[0xVPN] test packet sent OK")
    except Exception as e:
        click.echo(f"[0xVPN] WARNING: test packet failed: {e}")

    click.echo(f"[0xVPN] connected  peer={host}:{raw_port}  local={addr}")
    click.echo(f"[0xVPN] forwarding threads starting...")
    ...
    try:
        ...
        stop.set()
        if route_all and sys.platform == "darwin":
            click.echo(f"[0xVPN] restoring routes...")
            ...
            if original_dns:
                click.echo(f"[0xVPN] restoring DNS...")
                if original_dns == "empty":
                    subprocess.run(["networksetup", "-setdnsservers", "Wi-Fi", "empty"], check=False)
                else:
                    subprocess.run(["networksetup", "-setdnsservers", "Wi-Fi"] + original_dns, check=False)
        click.echo("\n[0xVPN] disconnected")
